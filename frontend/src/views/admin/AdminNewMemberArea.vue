<template>
    <div>
    <form @submit="join">
        <div class="p-6 flex flex-col bg-gray-100 my-4 rounded-lg shadow-sm w-8/12 mx-auto">
        <h3 class="normal-case text-3xl font-bold text-center">사원 생성</h3>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">사원명</label>
                <input v-model="joinInput.memberName" type="text" autocomplete="off" class="shadow border rounded py-2 px-3 font-bold text-xl text-gray-700 focus:outline-none focus:ring focus:border-blue-50" placeholder="사원명을 입력하세요.">
          </div>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">아이디</label>
            <input v-model="joinInput.memberId" type="text" 
          ...
          <div class="flex flex-col m-2">
            ...
            <select v-model="joinInput.departmentName"
                    ...
                    <option v-for="(deptName, idx) in joinFormData.departmentList" :key="idx" :selected="idx == 0">{{ deptName }}</option>
            </select>
          </div>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">연차 개수</label>
            <input v-model="joinInput.annualCount" type="number"
                   class="shadow border rounded py-2 px-3 text-gray-700 focus:outline-none focus:ring focus:border-blue-50"
                   autocomplete="off" placeholder="연차 개수를 입력하세요.">
          </div>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">입사일</label>
            <input v-model="joinInput.createdDate" type="date"
                  class="shadow border rounded py-2 px-3 text-gray-700 focus:outline-none focus:ring focus:border-blue-50"
                />
          </div>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">전화번호</label>
            <input v-model="joinInput.memberCellphone"  type="text" 
                   class="shadow border rounded py-2 px-3 text-gray-700 focus:outline-none focus:ring focus:border-blue-50"
                   autocomplete="off" placeholder="010-0000-0000">
          </div>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">이메일</label>
            <input v-model="joinInput.memberEmail" type="text" 
                   autocomplete="off"
                   class="shadow border rounded py-2 px-3 text-gray-700 focus:outline-none focus:ring focus:border-blue-50"
                   placeholder="이메일을 입력하세요.">
          </div>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">사용자 권한</label>
            <select v-model="joinInput.memberAuth"
                    class="shadow border rounded py-2 px-3 text-gray-700 hover:border-blue-300 focus:outline-none focus:ring focus:border-blue-50">
                    <option v-for="(auth, idx) in joinFormData.memberAuthList" :key="idx" :selected="idx == 0">{{ auth }}</option>
            </select>
          </div>
          <div class="flex flex-col m-2">
            <label class="text-gray-700 font-bold mb-2 text-xl">사용자 상태</label>
            <select v-model="joinInput.memberStatus"
                    class="shadow border rounded py-2 px-3 text-gray-700 hover:border-blue-300 focus:outline-none focus:ring focus:border-blue-50">
                    <option v-for="(status, idx) in joinFormData.memberStatusList" :key="idx" :selected="idx == 0">{{ status }}</option>
            </select>
          </div>
...
const join = (event) => {
  ...
  axios.post('http://localhost:8090/admin/member', toRaw(joinInput))
  .then(response => {
    alert("사원이 생성되었습니다.");
