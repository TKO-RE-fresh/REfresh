<template>
            ...
            <th>
              <div v-if="(row.annualStatus != '반려')">
                <button
                  ...
                  class="btn btn-active btn-link text-blue-300 disabled:text-gray-300 disabled:bg-inherit"  
              ...
              <div v-else>
                <button class="p-4 pl-6" @click="detailHandeler(row)">
                  <div>
  ...
  <RejectDetail :isOpen="isOpen" :onToggle="onToggle" :data="rejectData"></RejectDetail>
</template>
...
const cancelHandeler = (uid) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await axios
        .put("http://localhost:8090/myPage/history", { uid })
        .then(() => {
