...
public class AnnualRepositoryImpl {
    ...
    public Page<GetAnnualSearchDto> findAnnualByMember(GetAnnualByNameFilterDto dto) {
        ...
        q.setMaxResults(pageable.getPageSize());
        List<GetAnnualSearchDto> content = q.getResultList().stream()
                                            .map(annual -> GetAnnualSearchDto.builder().memberInfo(annual.getMember().getMemberInfo())
                                                                             .annualType(annual.getAnnualType()).period(annual.getPeriod()).build()).collect(Collectors.toList());
